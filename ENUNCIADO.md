...
### 🔴 O que não existe e você vai construir

- O módulo de vídeo, a tabela de vídeos, o serviço de object storage, a fila de processamento e o worker de vídeo (FFmpeg). A arquitetura-alvo (em `docs/diagrams/software-arch.mermaid` e no `CLAUDE.md`) já prevê esses três componentes como parte da Fase 03.
...
### 🗄️ Persistência
Uma entidade/tabela de vídeos ligada ao canal, com pelo menos identificação, dono (canal), título, status (ex.: rascunho → processando → pronto/erro), chaves de storage do arquivo e do thumbnail, duração e metadados, e o identificador da URL única. O modelo exato é definido no plano (Data Model).

### 🛠️ Decisões que você precisa tomar e justificar na etapa de research:
